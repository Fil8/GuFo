...
With the goal of enabling reproducible workflows for its users, GaNGiaLF employs as few as possible input files and a single configuration file. 

From terminal, a default configuration file can be generated in the working directory running ``gufo -gd``. The configuration file is made of a sequence of blocks, each of which allows to enable and set a GaNGiaLF joint, i.e. a set of
functions to generate a specific astrophysical observational analysis. 
...
and names of inputs and outputs. Other blocks are optional and should be provided only if the joints is enabled.
