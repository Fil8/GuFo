...
class cubeplay:
    ...
    def makeLineCubes(self,cfg_par):
        ...
        for ii in range(0,len(lineInfo['ID'])):
            ...
            velRange = float(lineInfo['lineRange'][ii])
            ...
            waveCube = hh['CRVAL3']+(np.arange(dCbl.shape[0]))*hh['CDELT3']
            waveCube = waveCube / (1+cfg_par['general']['redshift'])   
            print(waveCube)

            sys.exit(0)

            self.makeLineCube(cfg_par,lineName,lineNameStr,lineRange,ii,dd,hh)

        return

    def makeLineCube(self,lineName,lineNameStr,lineRange,ii,dd,hh):
